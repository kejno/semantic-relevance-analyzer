...
function assignForReview(ticketKey, initiatorId, wipLabel, targetStatus) {
    ...
    try {
        ...
        // Assign to initiator (non-fatal — initiatorId is empty/absent when the
        // SM agent triggered this run rather than a human, and jira_assign_ticket_to
        // throws on a missing accountId; that must not block the status move below)
        if (initiatorId) {
            try {
                ...
                console.warn('Failed to assign ' + ticketKey + ' to initiator:', assignError);
            }
        } else {
            console.log('No initiator id — skipping assign for ' + ticketKey);
        }
